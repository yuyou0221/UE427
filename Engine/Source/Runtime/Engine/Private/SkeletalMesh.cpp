...
#endif
	, bDrawOutlinePass(Component->DrawOutline)
	, OutlinePassMaterial(Component->OutlineMaterial)
{
	check(MeshObject);
	...
	else
	{
		...
		if (LODSections.Num() > 0 && LODIndex >= FirstLODIdx)
		{
			...
			for (FSkeletalMeshSectionIter Iter(LODIndex, *MeshObject, LODData, LODSection); Iter; ++Iter)
			{
				...
				if (bDrawOutlinePass && OutlinePassMaterial.Num() > SectionElementInfo.UseMaterialIndex && OutlinePassMaterial[SectionElementInfo.UseMaterialIndex]) {
					FSectionElementInfo OutlineInfo = FSectionElementInfo(SectionElementInfo);
					OutlineInfo.Material = OutlinePassMaterial[SectionElementInfo.UseMaterialIndex];
					GetDynamicElementsSection(Views, ViewFamily, VisibilityMap, LODData, LODIndex, SectionIndex, bSectionSelected, OutlineInfo, bInSelectable, Collector);
				}
